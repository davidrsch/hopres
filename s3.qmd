...
**Ejercicio 8.1 (Añade un Atributo)** Modifique `play` para devolver un premio que contenga los símbolos asociados a él como un atributo denominado `simbolos`. Elimine la llamada redundante a `print(simbolos)`:
...
**Ejercicio 8.2 (Hacer un método de print)** Escriba un nuevo método de print para la clase de tragamonedas. El método debería llamar a `mostrat_tragamoneda` para devolver una salida de máquina tragamonedas bien formateada.
...
**Ejercicio 8.3 (Añadir una Clase)** Modifique la función `play` para que asigne `tragamonedas` al atributo `class` de su salida:
