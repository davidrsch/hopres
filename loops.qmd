...
En segundo lugar, podemos buscar las probabilidades de obtener los valores en `Var2`:
...
**Ejercicio 9.1 (Liste las Combinaciones)** Usa `expand.grid` para crear un data frame que contenga todas las combinaciones posibles de *tres* símbolos del vector `rueda`:
...
**Ejercicio 9.2 (Haz una Tabla de Búsqueda)** Aísle las probabilidades anteriores en una tabla de búsqueda. ¿Qué nombres usarás en tu tabla?
...
**Ejercicio 9.3 (Buscar las Probabilidades)** Busque las probabilidades de obtener los valores en `Var1`. Luego agréguelos a `combos` como una columna llamada `prob1`. Luego haga lo mismo para `Var2` (`prob2`) y `Var3` (`prob3`).
...
**Ejercicio 9.4 (Calcular las Probabilidades de Cada Combinación)** Calcule las probabilidades generales para cada combinación. Guárdelos como una columna llamada `prob` en `combos`, luego verifique su trabajo.
...
**Ejercicio 9.5 (Construye un Bucle)** Construya un bucle `for` que ejecutará `puntuacion` en las 343 filas de `combos`. El ciclo debe ejecutar `puntuacion` en las primeras tres entradas de la \_i_ésima fila de `combos` y debe almacenar los resultados en la \_i_ésima entrada de `combos$premio`.
...
**Ejercicio 9.6 (Desafío)** Hay muchas formas de modificar `puntuacion` que contarían `DD` como comodines. Si desea probar su habilidad como programador de R, intente escribir su propia versión de `puntuacion` que maneje correctamente los diamantes.
...
**Ejercicio 9.7 (Calcular el Valor Esperado)** Calcule el valor esperado de la máquina tragamonedas cuando usa la nueva función `puntuacion`. Puede usar el data frame `combos` existente, pero necesitará crear un bucle `for` para recalcular `combos$premio`.
